...
module.exports = async function getPosts(site, all_posts, bot) { // todo module exports
  try {
    ...
    $('.sc-1tputnk-13.ixDVFm > div > a.sc-1tputnk-2').each( (_i , el ) => {
      ...
      if (postTitle && all_posts.filter(el => el.url == "https://www.kazan.kp.ru" + postTitle).length < 1) {
    ...
    await postTitles.reduce(async (memo, sitepost) => {
      await memo
      try {
        const { data } = await axios.get("https://www.kazan.kp.ru" + sitepost);
        const $ = cheerio.load(data);
        ...
        res.push({
          ...
          istochnik:`Источник: www.kazan.kp.ru`,
          siteid: site.id,
          old: false,
        });
